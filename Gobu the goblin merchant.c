...
int main() {
    ...
    // Sort and display the products by name
    printf("\nProducts sorted by Name:\n");
    sortByName(&Shop);
    displayProducts(&Shop);

    // Sort and display the products by id
...
void populateShop(GobuShop *shop) {
    ...
    shop->products[5] = (Product){7, "ScaleMail", 120.00};
    shop->products[6] = (Product){8, "LongBow", 10.00};
    shop->count = 7;
}

// Function to display the details of the products
void displayProducts(GobuShop *shop) {
...
// Function to sort the products in Gobu shop by name in alphabetical order
void sortByName(GobuShop *shop) {
    for (int i = 0; i < shop->count - 1; i++) {
        for (int j = 0; j < shop->count - i - 1; j++) {
...
// Function to sort the products in Gobu shop by id in descending order
void sortById(GobuShop *shop) {
    for (int i = 0; i < shop->count - 1; i++) {
        for (int j = 0; j < shop->count - i - 1; j++) {
